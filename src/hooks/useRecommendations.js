...
function mapToRestaurant(item) {
  const type =
    TYPE_MAP[item.recommendationType] ??
    TYPE_MAP[item.recommendationType?.toUpperCase()] ??
    "balance";

  const BADGE_MAP = {
    quality: "QUALITY PICK",
    balance: "BALANCE PICK",
    value: "VALUE PICK",
  };
  const TAG_MAP = {
    quality: "퀄리티 픽",
  ...
  const priceStr =
    avgPrice > 0 ? `${avgPrice.toLocaleString()}원` : "가격 미제공";
...
export function useRecommendations() {
  ...
  const reroll = useCallback(async (answers, excludedKakaoPlaceIds = []) => {
    ...
    setError(null);
    try {
      const data = await fetchRerollRecommendations(
        answers,

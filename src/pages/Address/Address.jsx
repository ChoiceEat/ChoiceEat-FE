...
const MOCK_RESULTS = [
  { id: 1, name: '창신동',   address: '서울 종로구 창신동' },
  { id: 2, name: '역삼동',   address: '서울 강남구 역삼동' },
  { id: 3, name: '연희동',   address: '서울 서대문구 연희동' },
  { id: 4, name: '합정동',   address: '서울 마포구 합정동' },
  { id: 5, name: '성수동',   address: '서울 성동구 성수동' },
  { id: 6, name: '홍대입구', address: '서울 마포구 서교동' },
  { id: 7, name: '이태원동', address: '서울 용산구 이태원동' },
  { id: 8, name: '신촌동',   address: '서울 서대문구 신촌동' },
];
...
export default function Address() {
  ...
  const SAMPLE_ADDRESSES = [
    { name: '창신동', address: '서울 종로구 창신동' },
    { name: '홍대입구', address: '서울 마포구 서교동' },
    { name: '성수동', address: '서울 성동구 성수동' },
  ];
  ...
  const activeAddress = JSON.parse(localStorage.getItem('savedAddress') || 'null');

  const handleSearch = () => {
    const q = query.trim();
    const results = q
      ? MOCK_RESULTS.filter(item => item.name.includes(q) || item.address.includes(q))
      : MOCK_RESULTS;
  ...
  const handleSelectAddress = (item) => {
    ...
    localStorage.setItem('savedAddresses', JSON.stringify([selected, ...deduped]));
    ...
    return (
      <div className="addr-container addr-results-view">
        <div className="addr-pill-wrap">
          <button className="addr-pill-back" onClick={handleBack} aria-label="뒤로가기">
          ...
          <input
            ...
            autoFocus
          />
          <button className="addr-pill-search" onClick={handleSearch} aria-label="검색">
            <img src="/icons/address-search.svg" alt="검색" className="addr-search-icon" />
          </button>
        </div>
        ...
        <ul className="addr-results">
          {filteredResults.length > 0 ? (
            filteredResults.map((item) => (
              <li key={item.id} className="addr-result-item" onClick={() => handleSelectAddress(item)}>
                <img src="/icons/location.svg" alt="위치" className="addr-result-pin" />
  ...
  return (
    <div className="addr-container">
      ...
      <div className="addr-search-wrap">
        ...
        <button className="addr-search-btn" onClick={handleSearch} aria-label="검색">
          <img src="/icons/address-search.svg" alt="검색" className="addr-search-icon" />
        </button>
      </div>
